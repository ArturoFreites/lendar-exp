...
import type { RegionResponse, PaginationResponse, StateResponse, CityResponse } from '../../services/api';
...
import { MapPin, RefreshCw, Search, Eye, Plus, Edit2, ArrowLeft, Upload } from 'lucide-react';
...
export function Regiones() {
  ...
  const [list, setList] = useState<RegionResponse[]>([]);
  ...
  const [detailItem, setDetailItem] = useState<RegionResponse | null>(null);
  ...
  const loadList = async (pageNum: number = 0, nameFilter?: string) => {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        const data = response.data as PaginationResponse<RegionResponse>;
        ...
        <div className="flex items-center gap-4 mb-6 shrink-0">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-[#3b3a3e] truncate">
              {editingId != null ? 'Editar región' : 'Nueva región'}
            </h1>
            <p className="text-sm text-[#6b6a6e]">
              {editingId != null ? 'Modificá los datos y las zonas de cobertura' : 'Completá nombre y zonas de cobertura'}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 p-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#3b3a3e]">Regiones</h1>
          <p className="text-[#6b6a6e] mt-1">Listado, creación y edición de regiones con cobertura</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalElements} región{totalElements !== 1 ? 'es' : ''} encontrada{totalElements !== 1 ? 's' : ''}
          </CardDescription>
        </CardHeader>
        <CardContent>
                          ...
                          <div className="flex gap-2 flex-shrink-0">
                            ...
                            {environment === 'development' && (
                              <Button
                                ...
                                className="gap-1"
                                title="Subir esta región a producción"
                              >
      ...
      <Dialog open={!!detailItem} onOpenChange={(open) => !open && setDetailItem(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Detalle de la región</DialogDescription>
          </DialogHeader>
          {detailItem && (
            <div className="space-y-4 py-4">
              ...
              {detailItem.address && Object.keys(detailItem.address).length > 0 && (
                <div>
                  <Label className="text-[#6b6a6e]">Dirección</Label>
                  <p className="text-sm text-[#3b3a3e] break-words">
                    {JSON.stringify(detailItem.address)}
                  </p>
                </div>
              )}
